...
function computeIndicator(candles) {
  ...
  const cciMa = sma(cciVal.map((v) => v ?? 0), settings.cci_ma_len).map((v, i) =>
    cciVal[i] == null ? null : v,
  );
  ...
  const cciAtr = sma(cciTr.map((v) => v ?? 0), 14).map((v, i) => (cciTr[i] == null ? null : v));
  ...
  for (let i = 0; i < candles.length; i += 1) {
    ...
    tCrossUp = Math.max(0, Math.min(1, tCrossUp));
    ...
    tEarly = Math.max(0, Math.min(1, tEarly));
    ...
    tCrossDown = Math.max(0, Math.min(1, tCrossDown));
    ...
    const condCciEarly =
      settings.enable_early &&
      isUnderMa &&
      gapUp <= finalEarly &&
      candle.close > candle.open &&
      isCciRising;
    const wasOverMa = (cciVal[i - 1] ?? 0) > (cciMa[i - 1] ?? 0);
    const condCciReentry =
      isOverMa &&
      wasOverMa &&
      isCciRising &&
      candle.close > (candles[i - 1]?.close ?? candle.close);
    ...
    if (triggerNewBuy) {
      ...
      buySignals.push({
        index: i,
        price: execBuy,
        reason: condCciEarly ? "⚡ 即將交叉" : condCciBuy ? "💥 黃金交叉" : "🚀 接續買點",
      });
...
function renderChart(stock) {
  ...
  if (!candles.length) {
    ...
    drawText("請匯入 `code,name,date,open,high,low,close,volume` 格式的 CSV", 60, 160, "#97a0af", 18);
    return;
  }
  ...
  const distStr = settings.enable_early
    ? `< ${round((settings.use_dynamic ? lastCciAtr * settings.early_mult : settings.static_early), 1)} 點`
    : "已關閉提前預判";

  const priceArea = { x: 40, y: 72, w: 890, h: 430 };
  const cciArea = { x: 40, y: 530, w: 890, h: 130 };
  const pnlArea = { x: 40, y: 690, w: 890, h: 120 };
  ...
  const visibleSt = computed.stValue.slice(-60);
  const visibleCci = computed.cciVal.slice(-60);
  const visibleCciMa = computed.cciMa.slice(-60);
  const visiblePnl = computed.livePnlHistory.slice(-60);
  const minPrice = Math.min(...visible.map((c) => c.low), ...visibleSt.filter(Boolean));
  const maxPrice = Math.max(...visible.map((c) => c.high), ...visibleSt.filter(Boolean));
  ...
  visible.forEach((candle, i) => {
    ...
    if (st != null) {
      ...
      ctx.fillStyle = computed.trend[offset + i] === -1 ? "#00ff9c" : "#ff4b4b";
      ctx.fillRect(x - 2, stY - 2, 4, 4);
      if (i > 0 && visibleSt[i - 1] != null) {
  ...
  [...computed.buySignals, ...computed.sellSignals]
    .filter((signal) => signal.index >= offset)
    .forEach((signal) => {
      ...
      const isBuy = Object.prototype.hasOwnProperty.call(signal, "reason") && !Object.prototype.hasOwnProperty.call(signal, "pnl");
      ...
      drawText(lines[0], x, boxY + 18, fg, 12, "center");
      drawText(lines[1], x, boxY + 34, fg, 12, "center");
      drawText(lines[2], x, boxY + 50, fg, 12, "center");
    });
  ...
  const rows = [
    ...
    ["當下 4H 幅度", `${round(liveKChange, 2)}%`, liveKChange >= settings.instant_surge ? "#103a28" : liveKChange <= -settings.instant_drop ? "#41111a" : "#111317", liveKChange >= settings.instant_surge ? "#15d18d" : liveKChange <= -settings.instant_drop ? "#ff5263" : "#ffffff"],
  ...
  drawText(formatDate(visible[0].date), priceArea.x, priceArea.y + priceArea.h + 30, "#97a0af", 12);
  drawText(formatDate(visible[visible.length - 1].date), priceArea.x + priceArea.w, priceArea.y + priceArea.h + 30, "#97a0af", 12, "right");
}
...
async function fetchTwseStockData(code) {
  ...
  const deduped = candles.filter((candle, index, array) => {
    if (index === 0) return true;
    return candle.date !== array[index - 1].date;
  });

  if (!deduped.length) {
    throw new Error("No official daily data");
  }
...
async function loadWatchlistRows(rows) {
  ...
  for (const row of rows) {
    ...
    if (!state.candlesByCode.has(row.code)) {
      // Load one by one to avoid hammering the official endpoint.
...
function loadPriceRows(rows) {
  ...
  if (!state.selectedCode && grouped.size) {
    state.selectedCode = [...grouped.keys()][0];
  }
  renderAll();
}
...
function generateDemoCandles(code, name, seed, startPrice) {
  ...
  for (let i = 0; i < 96; i += 1) {
    ...
    candles.push({
      ...
      close: round(close, 2),
      volume: Math.round(5000 + rand() * 9000),
    });
...
function loadDemoData() {
  ...
  generateDemoCandles("2317", "鴻海", 207, 164);
  generateDemoCandles("0050", "元大台灣50", 509, 186);
...
watchlistFileInput.addEventListener("change", (event) => {
  ...
  if (!file) return;
  readFile(file, (text) => {
    loadWatchlistRows(parseCsv(text));
  });
  event.target.value = "";
});
